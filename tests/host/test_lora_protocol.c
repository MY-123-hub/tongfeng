...
static const uint8_t G_TO_M1_READ[] = {
    0xAA, 0x55, 0x02, 0x01, 0x01, 0x00, 0x02,
    0x01, 0x64, 0x00, 0x01, 0x00, 0x3F, 0x13
};

static const uint8_t M1_TO_S1_READ[] = {
    0xAA, 0x55, 0x02, 0x01, 0x02, 0x01, 0x03,
    0x01, 0x64, 0x00, 0x01, 0x01, 0xAF, 0xD7
};

static const uint8_t S1_TO_M1_TEMP[] = {
    0xAA, 0x55, 0x02, 0x02, 0x03, 0x01, 0x02, 0x01, 0x64, 0x00, 0x4C,
    ...
    0xC8, 0x00, 0x00, 0x80, 0x80, 0x5F
};

static const uint8_t M1_TO_G_TEMP[] = {
    0xAA, 0x55, 0x02, 0x02, 0x02, 0x01, 0x01, 0x00, 0x64, 0x00, 0x4C,
    0xFA, 0x00, 0xFB, 0x00, 0xFC, 0x00, 0xFD, 0x00, 0xFE, 0x00, 0xFF, 0x00,
    0x00, 0x01, 0x01, 0x01, 0x02, 0x01, 0x03, 0x01, 0x04, 0x01, 0x05, 0x01,
    0x06, 0x01, 0x07, 0x01, 0x08, 0x01, 0x09, 0x01, 0x0A, 0x01, 0x0B, 0x01,
    0x0C, 0x01, 0x0D, 0x01, 0x0E, 0x01, 0x0F, 0x01, 0x10, 0x01, 0x11, 0x01,
...
static void TestMessageValidation(void)
{
    ...
    message = MakeMessage((uint8_t)LORA_MSG_ENV_DATA,
                          (uint8_t)LORA_ROLE_SLAVE,
                          1U,
                          (uint8_t)LORA_ROLE_MASTER,
                          1U,
                          LORA_PROTOCOL_ENV_PAYLOAD_SIZE);
    (void)memset(message.payload, 0xFF, LORA_PROTOCOL_ENV_PAYLOAD_SIZE);
    CHECK(LoRaProtocol_ValidateMessage(&message) == LORA_PROTOCOL_OK);
    CHECK(LoRaProtocol_Encode(&message, output, sizeof(output), &output_length) ==
          LORA_PROTOCOL_OK);
    CHECK(output_length == 99U);
    CHECK(output[2] == LORA_PROTOCOL_VERSION);
    CHECK(output[3] == (uint8_t)LORA_MSG_ENV_DATA);
    CHECK(output[10] == LORA_PROTOCOL_ENV_PAYLOAD_SIZE);
