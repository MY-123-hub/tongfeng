...
void LoraP2PRX(void)
{
  ...
  if(LORA_WaitRecive() == REV_OK)							//如果收到数据
  {
    if(strstr((const char *)Rx2Buffer, "PORT:") != NULL)		//如果检索到帧头:PORT:——温度数据关键词
    {
      
      /* 处理接受的 Lora 温度数据——作数据整形处理 */ 
      // 数据格式对比： PORT:00,NUM:04,TM:26.8/27.0/27.0/26.8/0.0/0.0,DHT11_H:60.0,DHT11_T:27.7,Pressure:31
      parse_count = sscanf((const char *)Rx2Buffer,"PORT:%02d,NUM:%02d,TM:%d.%d/%d.%d/%d.%d/%d.%d/%d.%d/%d.%d,HM:%d.%d/%d.%d/%d.%d/%d.%d/%d.%d/%d.%d,DHT11_H:%d.%d,DHT11_T:%d.%d,Pressure:%d",&LoRaType.DS18B20_PORT,&LoRaType.DS18B20_NUM,
              &Tdata[0],&Tdata[1],&Tdata[2],&Tdata[3],&Tdata[4],&Tdata[5],&Tdata[6],&Tdata[7],&Tdata[8],&Tdata[9],&Tdata[10],&Tdata[11],
              &Tdata[12],&Tdata[13],&Tdata[14],&Tdata[15],&Tdata[16],&Tdata[17],&Tdata[18],&Tdata[19],&Tdata[20],&Tdata[21],&Tdata[22],&Tdata[23],
              &Tdata[24],&Tdata[25],&Tdata[26],&Tdata[27],&Tdata[28]);
      if ((parse_count != 31) || (LoRaType.DS18B20_NUM <= 0) ||
      ...
      for(uint8_t i=0;i<6;i++){LoRaType.HM_Data[i]=Tdata[12+i*2]*10+Tdata[12+i*2+1];}
      LoRaType.DHT11_Humi=Tdata[24]*10+Tdata[25];
      LoRaType.DHT11_Temp=Tdata[26]*10+Tdata[27];
      LoRaType.WindPressure=Tdata[28]*10;

#if 0     // Debug output (disable to prevent noise dump to USART1)
      /* print raw data and parsed values */
      HAL_UART_Transmit(&huart1,(const unsigned char *)Rx2Buffer,strlen((const char *)Rx2Buffer),50);
      printf("PORT:%02d,NUM:%02d\r\n",LoRaType.DS18B20_PORT,LoRaType.DS18B20_NUM);
      printf("TM:%d.%d/%d.%d/%d.%d/%d.%d/%d.%d/%d.%d\r\n",
        Tdata[0],Tdata[1],Tdata[2],Tdata[3],Tdata[4],Tdata[5],
        Tdata[6],Tdata[7],Tdata[8],Tdata[9],Tdata[10],Tdata[11]);
      printf("HM:%d.%d/%d.%d/%d.%d/%d.%d/%d.%d/%d.%d\r\n",
        Tdata[12],Tdata[13],Tdata[14],Tdata[15],Tdata[16],Tdata[17],
        Tdata[18],Tdata[19],Tdata[20],Tdata[21],Tdata[22],Tdata[23]);
      printf("DHT11_H:%d.%d,DHT11_T:%d.%d,Pressure:%d\r\n",
        Tdata[24],Tdata[25],Tdata[26],Tdata[27],Tdata[28]);
